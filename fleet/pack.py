...
def discover(data_dir, date, session=None):
    """Return room dicts for this date across the date's per-session folders. v2 layout:
    data/{date}_{HHMM}/{anchor}/ — the anchor dir IS the room session (files live directly inside).
    Also accepts the legacy single-folder layout data/{date}/{anchor}/.

    If `session` (a single folder name like '20260819_2000') is given, ONLY that folder is packed —
    so a manual run doesn't sweep in other same-day sessions (e.g. a nightly run's data)."""
    rooms = []
    root = Path(data_dir)
    if session:
        session_dirs = [root / session] if (root / session).is_dir() else []
    else:
        # every session folder for this date: data/{date}_HHMM/ (new) + data/{date}/ (legacy)
        session_dirs = sorted(p for p in root.glob(f"{date}_*") if p.is_dir())
        if (root / date).is_dir():
            session_dirs.append(root / date)
    for sess in session_dirs:
        for anchor_dir in sorted(p for p in sess.iterdir() if p.is_dir()):
            ...
            if not meta_path.exists():
                ...
                if m and (sess / m.group(1) / "meta.json").exists():
                    meta_path = sess / m.group(1) / "meta.json"
            if meta_path.exists():
            ...
            # session key must be unique across folders: same room recorded in two sessions of the
            # same date shares an anchor name, so scope it by the session folder ({stamp}/{anchor}).
            rooms.append(dict(room_id=room_id, live_id=live_id, name=name,
                              anchor_dir=anchor_dir, session=f"{sess.name}/{anchor_dir.name}",
...
def main():
    ...
    ap.add_argument("--session", help="pack only this session folder (YYYYMMDD_HHMM); "
                                      "default = every session of --date")
    args = ap.parse_args()
    ...
    rooms = discover(args.data_dir, date, session=args.session)
    if not rooms:
        where = args.session or f"date={date}"
        print(f"[pack] no sessions for {where} in {args.data_dir}", file=sys.stderr)
        sys.exit(1)
